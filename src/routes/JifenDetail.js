...
export default class JifenDetail extends Component {
  ...
  componentWillMount(){
    ...
    if(this.cwt){
      ...
      let goodDetail=shopData.jifenDetailData;
      ...
      // console.log(this.state.selected,'挂载后');
    }
  }
  componentWillUnmount(){
    this.cwt=false;  //
    this.setState({selected:[]});
    // console.log(this.state.selected,'卸载前');
  }
  //
  componentWillReceiveProps(nextProps){
    const {shopData}=nextProps;
    let goodDetail=shopData.jifenDetailData;
    // console.log(goodDetail,'是轮播图？');
    if(goodDetail.specifications){
      let selected=[];
      goodDetail.specifications.map((i,index)=>{
        let item={name:i.goodsPropertyCategoryName,value:i.goodsPropertyList[0].goodsPropertyName};
  ...
  async buyNow(){
    ...
    console.log(goods._id,'嘻嘻嘻');
    let _id=goods._id;
    const value=await fetch.cunSaveOrder(sql);
    if(value.status){
      Toast.success(value.message,2,()=>{
        dispatch(routerRedux.push('/cop?_id='+_id))
      });
    }else{
      Toast.fail(value.message,2);
    }
  ...
  render() {
    ...
    const {history,dispatch,shopData}=this.props;
    console.log(shopData,'w ca.?s');
    const {num,propModal,selected,price,stock,s_oldPrice}=this.state;
    console.log(selected,'????噢哟？');
    let goodDetail=shopData.jifenDetailData;
    // console.log(goodDetail.model,s_oldPrice,stock,'xxxxx@');
    ...
    // console.log(module,propertyPrice,propertyOriginalPrice,'出来？');
    ...
    const _this=this;
    // console.log(price,goodDetail.availableIntegral,goodDetail.originalIntegral,systemConfig.ysjfDKRate,'fuck');
    console.log(goodDetail,'噢哟？');
          ...
          goodDetail._id?
            <div className={styles.main}>
              ...
              <div className={styles.goodInfo}>
                ...
                <p className={styles.price}>积分{propertyPrice} <span>抵扣价</span><span>(使用积分后)</span></p>
                <p className={styles.oldPrice}>当前价：<span>{propertyPrice}</span>原价：<span>{propertyOriginalPrice}</span>已兑量：<span>{goodDetail.sales}</span></p>
              ...
              <div className={styles.tab}>
                ...
                {/* 按钮组件 */}
                <div className={styles.imsBox}>
                  <button onClick={()=>this.buyNow()}>兑换</button>
                </div>
